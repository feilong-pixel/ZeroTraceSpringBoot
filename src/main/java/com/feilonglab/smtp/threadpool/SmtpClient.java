package com.feilonglab.smtp.threadpool;
 ...
 * 1. <b>Spring Boot 读取方式：</b>当作为 Spring Bean 注入时，利用
 * {@link org.springframework.context.annotation.PropertySource} 显式指定类路径下的
 * {@code mail.properties} 资源，使其属性合并至 Spring {@code Environment} 中， 各字段通过
 * {@code @Value} 注解从 Spring 环境中自动注入。<br>
 * 2. <b>管理模式：</b>配置了 {@code @Component} 与 {@code @Scope("prototype")}，允许使用
...
public class SmtpClient implements AutoCloseable {
	...
	@Value("${mail.smtp.send.debug.flag:1}")
	private String sendDebugFlag = SendDebugFlag.SEND_SUCCESS.getValue();

	/**
	 * 默认构造函数。 在 Spring 容器管理模式下，属性值通过 @Value 注解由容器自动注入。
	 */
	public SmtpClient() {
		// 无需手动读取 properties，由 Spring 容器自动通过 @Value 注入
	}
