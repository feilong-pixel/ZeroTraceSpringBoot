...
public class SmtpClient implements AutoCloseable {
	...
	/** SMTP 配置文件路径，默认位于类路径下的 "/mail.properties" */
	...
	/**
	 * 默认构造函数。 自动尝试从类路径下的固定配置文件路径 "/mail.properties" 加载 SMTP 属性配置。
	...
	public void loadConfig(String configFilePath) {
		logger.info(MessageUtils.getMessage("smtp.config.attempt", configFilePath));

		// 使用 Properties 对象加载配置文件
		...
		try (InputStream in = getClass().getResourceAsStream(configFilePath)) {
			if (in == null) {
				logger.warn(MessageUtils.getMessage("smtp.config.missing", configFilePath));
				return;
			}
			props.load(in);
			logger.info(MessageUtils.getMessage("smtp.config.success", configFilePath));
		} catch (Exception e) {
			logger.warn(MessageUtils.getMessage("smtp.config.failed", configFilePath, e.getMessage()));
			return;
		}
		...
		this.debugFlag = props.getProperty("mail.smtp.debug.flag", SmtpDebugFlag.REAL_CONNECTION.getValue());
		this.sendDebugFlag = props.getProperty("mail.smtp.send.debug.flag", SendDebugFlag.SEND_SUCCESS.getValue());
	}
