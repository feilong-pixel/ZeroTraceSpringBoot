 ...
 * SMTP 服务器连接调试标志枚举。
 */
public enum SmtpDebugFlag {

    /** 关闭拦截（真实连接） */
    REAL_CONNECTION("0", "关闭拦截（真实连接）"),

    /** 开启拦截（模拟连接与发送） */
    SIMULATION("1", "开启拦截（模拟连接与发送）");
    ...
    /** 构造函数 */
    SmtpDebugFlag(String value, String text) {
        this.value = value;
        this.text = text;
    }
     ...
     */
    public static SmtpDebugFlag fromValue(String value) {
        for (SmtpDebugFlag flag : SmtpDebugFlag.values()) {
